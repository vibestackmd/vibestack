...
user.settings.json        # User-level settings template (voiceEnabled, statusLine, defaultMode, companyAnnouncements, etc) — fetched by install.sh and deep-merged into ~/.claude/settings.json. Plugin enablement is NOT listed here — chain-install via plugin.json `dependencies` is the single source of truth.
