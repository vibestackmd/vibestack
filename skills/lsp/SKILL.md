...
This skill teaches Claude when and how to reach for LSP via shell commands (`tsc`, `pyright`, `cargo check`, `go vet`) and the LSP request types it should use when a deeper integration is wired up. The shell commands work as long as the language server binary is on `$PATH` (the VibeStack `dev-tools` installer can set these up). For richer in-process LSP integration, install the matching official plugin once: `/plugin install typescript-lsp@claude-plugins-official` (or `pyright-lsp`, `rust-analyzer-lsp`, `gopls-lsp`). When VibeStack is installed via `/plugin install vibestack`, those plugins chain-install automatically.
